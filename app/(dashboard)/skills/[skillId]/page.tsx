    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="relative overflow-hidden border-b border-slate-800/50">
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div className="absolute bottom-0 left-1/4 w-96 h-96 bg-violet-500/20 rounded-full blur-3xl animate-hero-reveal" style={{ animationDelay: '0.2s' }} />
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-xl p-6">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <span className="text-sm text-slate-400">Proficiency Level</span>
                    <h3 className="text-2xl font-bold text-white">
                      {getProficiencyLabel(skill.proficiencyLevel)}
                    </h3>
                  </div>
                  <div className="text-right">
                    <span className="text-4xl font-bold" style={{ color: proficiencyColor }}>
                      {skill.strength}%
                    </span>
                    <div className="flex items-center gap-1 justify-end mt-1">
                      <TrendingUp className="w-4 h-4 text-emerald-400" />
                      <span className="text-sm text-emerald-400">+{skill.trend.changePercentage}%</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-xl p-4">
                ...
                <p className="text-2xl font-bold text-white">{skill.stats.totalProjects}</p>
              </div>
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-xl p-4">
                ...
                <p className="text-2xl font-bold text-white">{skill.stats.totalHours.toLocaleString()}</p>
              </div>
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-xl p-4">
                ...
                <p className="text-2xl font-bold text-white">{skill.stats.endorsements}</p>
              </div>
              ...
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-xl p-4">
                ...
                <p className="text-2xl font-bold text-white">{skill.stats.certifications}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-8 animate-tab-switch">
            ...
            {growth && (
              <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
                <h2 className="text-xl font-bold text-white mb-6">Growth Over Time</h2>
                <div className="h-64 flex items-end gap-2">
                  {growth.history.map((point, index) => (
                    <div key={index} className="flex-1 flex flex-col items-center gap-2">
                      ...
                      />
                      {index % 2 === 0 && (
                        <span className="text-xs text-slate-500">{point.date.toLocaleDateString('en-US', { month: 'short' })}</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {/* Related Skills */}
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
              <h2 className="text-xl font-bold text-white mb-6">Related Skills</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {skill.relatedSkills.map((relatedSkill) => (
                  ...
                  >
                    <div className="flex-1">
                      <h3 className="text-sm font-medium text-white">{relatedSkill.name}</h3>
                      <p className="text-xs text-slate-400 capitalize">{relatedSkill.relationship}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold text-cyan-400">{relatedSkill.strength}%</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="animate-carousel-slide">
                  {evidence[carouselIndex] && (
                    <div>
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-cyan-500/20 to-violet-500/20 flex items-center justify-center">
                            {evidence[carouselIndex].type === EvidenceType.PROJECT && <Briefcase className="w-6 h-6 text-cyan-400" />}
                            {evidence[carouselIndex].type === EvidenceType.CERTIFICATION && <Award className="w-6 h-6 text-amber-400" />}
                            {evidence[carouselIndex].type === EvidenceType.CONTRIBUTION && <Users className="w-6 h-6 text-violet-400" />}
                            {evidence[carouselIndex].type === EvidenceType.AWARD && <Star className="w-6 h-6 text-yellow-400" />}
                          </div>
                          <div>
                            <span className="text-xs text-slate-400">{getEvidenceTypeLabel(evidence[carouselIndex].type)}</span>
                            <h3 className="text-2xl font-bold text-white">{evidence[carouselIndex].title}</h3>
                          </div>
                        </div>
                      ...
                      <p className="text-slate-300 mb-6">{evidence[carouselIndex].description}</p>
                      
                      {evidence[carouselIndex].metadata.metrics && (
                        <div className="grid grid-cols-3 gap-4 mb-6">
                          {evidence[carouselIndex].metadata.metrics?.map((metric, index) => (
                      ...
                      <div className="flex items-center justify-between text-sm text-slate-400">
                        <span>{evidence[carouselIndex].metadata.organization}</span>
                        <span>{formatDaysAgo(evidence[carouselIndex].date)}</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {evidence.map((_, index) => (
                  <button
                    ...
                    className={`
                      ...
                      ${index === carouselIndex ? 'w-8 bg-cyan-400' : 'bg-slate-600'}
                    `}
                  />
                ))}
              </div>
              ...
              {evidence.map((item, index) => (
                ...
                >
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-cyan-500/20 to-violet-500/20 flex items-center justify-center flex-shrink-0">
                    {item.type === EvidenceType.PROJECT && <Briefcase className="w-5 h-5 text-cyan-400" />}
                    {item.type === EvidenceType.CERTIFICATION && <Award className="w-5 h-5 text-amber-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-medium text-white truncate">{item.title}</h4>
                    <p className="text-xs text-slate-400">{formatDaysAgo(item.date)}</p>
                  </div>
                </button>
              ))}
            </div>
            ...
            {endorsements.map((endorsement, index) => (
              ...
              >
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-semibold text-white">{endorsement.endorserName}</h4>
                    <p className="text-xs text-slate-400">{endorsement.endorserRole}</p>
                    <p className="text-xs text-slate-500">{endorsement.endorserCompany}</p>
                  </div>
                ...
                {endorsement.testimonial && (
                  <p className="text-sm text-slate-300 italic mb-4">"{endorsement.testimonial}"</p>
                )}
            ...
            {learningResources.map((resource) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      {resource.type === ResourceType.COURSE && <Play className="w-4 h-4 text-cyan-400" />}
                      {resource.type === ResourceType.DOCUMENTATION && <FileText className="w-4 h-4 text-violet-400" />}
                      {resource.type === ResourceType.PROJECT && <Briefcase className="w-4 h-4 text-emerald-400" />}
                    ...
                    <p className="text-xs text-slate-400 mb-3">{resource.provider}</p>
                  </div>
                  {resource.price === 0 ? (
                    <span className="px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded text-xs font-medium">Free</span>
                  ) : (
                ...
                {resource.progress !== undefined && (
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-cyan-400">{resource.progress}%</span>
                    </div>
